...
//         console.log("um numero entre 1 e 10 por favor")
...
//     const agora = new Date(); // Captura o instante exato
...
//const pegarAnoAtual = () => {
//    const agora = new Date(); // Agora o JS sabe de onde tirar a informação
//    const ano = agora.getFullYear(); // Pega o ano com 4 dígitos (ex: 2026)
...
//     }
//     else {
//         console.log(`numero ${num2} e ${num1} são iguais`)
//     }
// }
...
// const calcularJurosMensais = (valorI, valorA, tempoM) => {
    
   

//     const taxa = Math.pow((valorA / valorI), (1 / tempoM)) - 1;    
//     const taxaPorcentagem = taxa * 100;

//     return taxaPorcentagem.toFixed(2).replace('.', ',') + '%';
// }
...
const calcularDesconto = (valorCompra, primeiraCompra, pagamentoAVista) => {
    let desconto = 0;
    ...
    if (desconto > 0) {
        ...
        console.log(`Você recebeu ${desconto * 100}% de desconto!`);
    } else {
        const cupomAleatorio = Math.floor(Math.random() * (20 - 10 + 1)) + 10;
        console.log(`Valor total: R$ ${valorCompra.toFixed(2)}`);
        console.log(`Infelizmente você não teve desconto hoje, mas aqui está um cupom de ${cupomAleatorio}% para sua próxima compra!`);
    }
};

// Teste: Valor 1200, Primeira Compra (true), À Vista (true)
